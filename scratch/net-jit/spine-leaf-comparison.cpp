...
void PrintTopologyAndApps(uint32_t leaf_count, uint32_t servers_per_leaf, uint32_t spine_count) {
    std::cout << "\n================================================================================\n";
    ...
    std::cout << "[Application Deployment Map (showing first 4 Leaves as example)]\n";
    
    uint32_t display_leaves = std::min((uint32_t)4, leaf_count);
    for (uint32_t l = 0; l < display_leaves; ++l) {
    ...
    if (leaf_count > display_leaves) {
        std::cout << "  ... (Leaf " << display_leaves << " to " << leaf_count - 1 << " follow the identical symmetric interleaved pattern)\n";
    }
...
std::vector<MoeTask> GetTasksForRank(MPIRankIDType rank) {
    ...
    if (rank >= (MPIRankIDType)g_num_nodes) return rank_tasks;
    ...
    int logical_node_count = g_leaf_count * nodes_per_app_per_leaf;
    ...
    auto PhysicalRank = [](int logical_id, int app) {
        int l_idx = logical_id / (g_servers_per_leaf / 4);
        int local_idx = logical_id % (g_servers_per_leaf / 4);
        return l_idx * g_servers_per_leaf + (app * (g_servers_per_leaf / 4)) + local_idx;
    };
...
// 开启包粒度的 ECMP 路由
    ...
    uint32_t leaf_count = 4;
    uint32_t servers_per_leaf = 4;
    uint32_t spine_count = 2;
    std::string modeStr = "aging";
    bool print_topo = false;
    double scale_factor = 0.001; 
    double app1_volume_factor = 50.0;
    double app2_volume_factor = 50.0;
    ...
    cmd.AddValue ("scale_factor", "Proportionally scale down data volume and link bandwidth (e.g. 0.001 for faster simulation)", scale_factor);
    ...
    if (print_topo) {
        PrintTopologyAndApps(leaf_count, servers_per_leaf, spine_count);
    }
    
    int app_node_count = g_num_nodes / 4;
    ...
    config_base.max_events = 2;//修改这里控制每个任务all2allv的数量
    ...
    NS_LOG_INFO("Running Spine-Leaf Comparison test in mode: " << modeStr << " | Nodes: " << g_num_nodes);
    
    SpineLeafTopology topo(leaf_count, servers_per_leaf, spine_count, bandwidth, delay);

    MoeJITApplicationHelper moeHelper;
    moeHelper.SetRunContext(modeStr, bandwidth, g_num_nodes);
    moeHelper.SetTraceLoader(GetTasksForRank);
    
    std::shared_ptr<MoeScheduler> scheduler;
    if (modeStr == "ideal") {
        auto idealSched = std::make_shared<CentralizedAdmissionScheduler>(scale_factor, topo.servers.GetN());
        ...
        scheduler = std::make_shared<DistributedAgingScheduler>(servers_per_leaf, leaf_count, mode, scale_factor); 
    }
    ...
    Simulator::Stop(Seconds(100000.1));
