...
std::vector<int> batchFlag = {-1, -1, -1, -1};
std::vector<std::vector<int>> batchFlags = {{-1, -1, -1, -1}, {-1, -1, -1, -1}};
...
main(int argc, char** argv)
{
    ...
    std::string tracePath2 = "scratch/net-jit/traces/resnet152";
//    std::string measure_traceFilePath2 = "scratch/net-jit/net-jit-traces/gpt2/1.txt";
    std::string measure_traceFilePath2 = "scratch/net-jit/net-jit-traces/resnet152/1.txt";
    ...
    bool usingVarys = false;

    // double threshold = 0.00000001;

    // set default ttl        for (auto& [rank, node] : nodes)
    //        {
    ...
    // todo:: 调整带宽
    //  change bandwidth
    bandwidth = std::to_string(scale_factor * 1.0) + "Gbps";
    ...
    // 生成函数helper
    ns3::ApplicationContainer applications1;
    ns3::ApplicationContainer applications2;
    if (usingVarys)
    {
        applications1 = dmlappHelper1.empty_install(masterRankID,
                                                    callbacks1,
                                                    distribute_generator1,
                                                    maxBytes,
                                                    scale_factor);
        applications2 = dmlappHelper2.empty_install(masterRankID,
                                                    callbacks2,
    ...
    {
        applications1 = dmlappHelper1.install_alltoallv(masterRankID,
                                              callbacks1,
                                              distribute_generator1,
                                              maxBytes,
                                              scale_factor);
        applications2 = dmlappHelper2.install_alltoallv(masterRankID,
                                              callbacks2,
                                              distribute_generator2,
                                              maxBytes,
                                              scale_factor);
    ...
    applications2.Start(ns3::Seconds(start_time));
