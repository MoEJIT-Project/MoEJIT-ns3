...
// Global Caches for Scaled Events (8 Apps)
...
static std::vector<MoeGlobalEvent> g_app5_events;
static std::vector<MoeGlobalEvent> g_app6_events;
static std::vector<MoeGlobalEvent> g_app7_events;
static std::vector<MoeGlobalEvent> g_app8_events;
...
void PrintTopologyAndApps(uint32_t leaf_count, uint32_t servers_per_leaf, uint32_t spine_count, uint32_t active_leaf_count) {
    ...
    int nodes_per_app_per_leaf = servers_per_leaf / 8; // Each of 8 apps gets its own server
    if (nodes_per_app_per_leaf == 0) nodes_per_app_per_leaf = 1; 
    ...
    for (uint32_t l = 0; l < active_leaf_count; ++l) {
        ...
        for (uint32_t s = 0; s < servers_per_leaf; ++s) {
            ...
            else if (app_id == 4) app_name = "App5(T5_b90)";
            else if (app_id == 5) app_name = "App6(BERT_b90)";
            else if (app_id == 6) app_name = "App7(GPT_b50)";
            else if (app_id == 7) app_name = "App8(T5_b50)";
...
std::vector<MoeTask> GetTasksForRank(MPIRankIDType rank) {
    ...
    // Divide servers within a leaf into 8 segments for 8 apps
    int nodes_per_app_per_leaf = g_servers_per_leaf / 8; 
    if (nodes_per_app_per_leaf == 0) nodes_per_app_per_leaf = 1;
    int app_id = local_index_in_leaf / nodes_per_app_per_leaf; // 0 to 7
    
    int logical_rank = leaf_index * nodes_per_app_per_leaf + (local_index_in_leaf % nodes_per_app_per_leaf);
    // Logical node count for EACH app
    ...
    if (app_id == 0) {
        ...
        current_events = &g_app5_events;      // APP5: T5 b90
        task_id_offset = 400000;
    } else if (app_id == 5) {
        current_events = &g_app6_events;      // APP6: BERT b90
        task_id_offset = 500000;
    } else if (app_id == 6) {
        current_events = &g_app7_events;      // APP7: GPT b50
        task_id_offset = 600000;
    } else if (app_id == 7) {
        current_events = &g_app8_events;      // APP8: T5 b50
        task_id_offset = 700000;
    }
...
int main(int argc, char *argv[])
{
    ...
    double app5_volume_factor = 1.0;
    double app6_volume_factor = 1.0;
    double app7_volume_factor = 1.0;
    double app8_volume_factor = 1.0;
    int max_events = 2;
    uint32_t active_leaf_count = 4; // 0 means use all leaves (leaf_count)
    ...
    cmd.AddValue ("app5_volume", "Volume factor for App 5 (T5 b90)", app5_volume_factor);
    cmd.AddValue ("app6_volume", "Volume factor for App 6 (BERT b90)", app6_volume_factor);
    cmd.AddValue ("app7_volume", "Volume factor for App 7 (GPT b50)", app7_volume_factor);
    cmd.AddValue ("app8_volume", "Volume factor for App 8 (T5 b50)", app8_volume_factor);
    cmd.AddValue ("max_events", "Max events per application (replay enabled)", max_events);
    cmd.AddValue ("active_leaves", "Number of active leaves (0 for all)", active_leaf_count);
    cmd.AddValue ("scale", "Scale factor for bandwidth and volume", scale_factor);
    cmd.Parse (argc, argv);
    ...
    if (active_leaf_count == 0 || active_leaf_count > leaf_count) {
        g_active_leaf_count = leaf_count;
    } else {
        g_active_leaf_count = active_leaf_count;
    }
    ...
    // Each application is installed on exactly one server per active leaf
    uint32_t active_nodes = g_active_leaf_count * servers_per_leaf;
    int app_node_count = g_active_leaf_count; 
    ...
    config_base.target_n = app_node_count; 
    config_base.volume_scale = scale_factor;
    config_base.computation_scale = scale_factor;
    config_base.max_events = max_events; // From command line argument
    ...
    // App 5 to 8 (New)
    MoeTraceParser::TraceConfig config_app5 = config_base;
    config_app5.target_batch = 90;
    config_app5.volume_scale = scale_factor * app5_volume_factor;
    config_app5.computation_scale = scale_factor * app5_volume_factor;

    MoeTraceParser::TraceConfig config_app6 = config_base;
    config_app6.target_batch = 90;
    config_app6.volume_scale = scale_factor * app6_volume_factor;
    config_app6.computation_scale = scale_factor * app6_volume_factor;

    MoeTraceParser::TraceConfig config_app7 = config_base;
    config_app7.target_batch = 50;
    config_app7.volume_scale = scale_factor * app7_volume_factor;
    config_app7.computation_scale = scale_factor * app7_volume_factor;

    MoeTraceParser::TraceConfig config_app8 = config_base;
    config_app8.target_batch = 50;
    config_app8.volume_scale = scale_factor * app8_volume_factor;
    config_app8.computation_scale = scale_factor * app8_volume_factor;
    ...
    g_app5_events = MoeTraceParser::GenerateScaledEvents(tpl_t5, config_app5);
    g_app6_events = MoeTraceParser::GenerateScaledEvents(tpl_bert, config_app6);
    g_app7_events = MoeTraceParser::GenerateScaledEvents(tpl_gpt, config_app7);
    g_app8_events = MoeTraceParser::GenerateScaledEvents(tpl_t5, config_app8);
    ...
    if (modeStr == "ideal") {
        ...
        // m_k=servers_per_leaf must be kept for correct srcLeaf=rank/m_k mapping.
        // But m_capacityPerSlot = 125000*scale*m_k = 1000 bytes/slot is 8x too high
        // because each app only has 1 node per leaf (not 8).
        // Fix: construct with correct m_k, then override m_capacityPerSlot to 1/4.
        // (1/8 caused search explosion due to sub-byte capacity per slot)
        auto agingSched = std::make_shared<DistributedAgingScheduler>(servers_per_leaf, g_active_leaf_count, mode, scale_factor);
        // 【FIX】Set to 1/4 of default: enough to cause real queuing without blowing up search
        agingSched->m_capacityPerSlot = static_cast<uint32_t>(125000 * scale_factor * 2);
        if (agingSched->m_capacityPerSlot == 0) agingSched->m_capacityPerSlot = 1;
        // Also reduce search window to keep scheduling fast
        agingSched->m_Wsearch = 200;
        scheduler = agingSched;
    }
