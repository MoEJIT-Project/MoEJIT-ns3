...
main(int argc, char** argv)
{
    ...
    std::string measure_method =
        "net-jit"; // "net-jit" or "static_window" or "slide_window" or "none" or "ideal"
    ...
    cmd.AddValue("measure_method",
                 "the measure method : \"net-jit\" or \"static_window\" or \"slide_window\" or "
                 "\"none\" or \"ideal\"",
                 measure_method);
    ...
    // other links down between switches
    for (SwitchID switch_index1 = 0; switch_index1 < dragonfly.switchCount; ++switch_index1)
    {
        for (SwitchID switch_index2 = switch_index1 + 1; switch_index2 < dragonfly.switchCount;
             ++switch_index2)
        {
            if (dragonfly.backgroundLinkMap.find(switch_index1) !=
                    dragonfly.backgroundLinkMap.end() &&
                dragonfly.backgroundLinkMap[switch_index1].find(switch_index2) !=
                    dragonfly.backgroundLinkMap[switch_index1].end() &&
                dragonfly.backgroundLinkMap.find(switch_index2) !=
                    dragonfly.backgroundLinkMap.end() &&
                dragonfly.backgroundLinkMap[switch_index2].find(switch_index1) !=
                    dragonfly.backgroundLinkMap[switch_index2].end())
            {
                continue;
            }
            auto tor1 = dragonfly.switches.Get(switch_index1);
            auto tor2 = dragonfly.switches.Get(switch_index2);
            auto tor1Ipv4 = tor1->GetObject<ns3::Ipv4>();
            auto tor2Ipv4 = tor2->GetObject<ns3::Ipv4>();
            auto tor1Ipv4L3Protocol = tor1->GetObject<ns3::Ipv4L3Protocol>();
            auto tor2Ipv4L3Protocol = tor2->GetObject<ns3::Ipv4L3Protocol>();
    ...
    //                   "network devices of tor are not installed correctly"); // with an
    //                   additional
    //                                                                          // loopback device
    // }
    ...
    PredictionCallback prediction_callback =
        [](ns3::MPIRankIDType rank, ns3::BatchID batch, TimeType time, ns3::SizeType size) {
            // do nothing
        };
    ...
    if (measure_method == "net-jit")
    {
        prediction_callback = [&dragonfly](ns3::MPIRankIDType rank,
                                           ...
                                           TimeType time,
                                           ns3::SizeType size) {
            // dragonfly.prediction_callback_for_MUSE(
            //     rank,
            //     batch,
            //     time,
    ...
    BatchCallback batch_communication_start_callback = [](ns3::MPIRankIDType rankid,
                                                          ns3::BatchID batchid) {
        record_into_file("batch_communication_start.txt",
                         std::to_string(ns3::Simulator::Now().GetNanoSeconds()) + " " +
                             std::to_string(rankid) + " " + std::to_string(batchid));
    };
    BatchCallback batch_communication_end_callback = [](ns3::MPIRankIDType rankid,
                                                        ns3::BatchID batchid) {
    ...
    {
        batch_communication_start_callback = [&dragonfly, &traces](ns3::MPIRankIDType rankid,
                                                                   ns3::BatchID batchid) {
            record_into_file("batch_communication_start.txt",
                             std::to_string(ns3::Simulator::Now().GetNanoSeconds()) + " " +
                                 std::to_string(rankid) + " " + std::to_string(batchid));
            dragonfly.multiple_incremental_changes(dragonfly.flows1,
                                                   batchid,
                                                   0);
        };

        batch_communication_end_callback = [&dragonfly, &traces](ns3::MPIRankIDType rankid,
                                                                 ns3::BatchID batchid) {
            record_into_file("batch_communication_end.txt",
                             std::to_string(ns3::Simulator::Now().GetNanoSeconds()) + " " +
                                 std::to_string(rankid) + " " + std::to_string(batchid));
            dragonfly.clear_flows(dragonfly.flows1, traces.predictions[0].size, batchid);
        };
    }
    ...
    if (measure_method == "net-jit")
    {
        batch_communication_start_callback = [&dragonfly, &traces](ns3::MPIRankIDType rankid,
            ...
            ns3::Simulator::Schedule(ns3::Seconds(random_delay),
    ...
    if (measure_method == "static_window")
    {
        for (auto& batch : traces.batches)
        {
            ns3::Simulator::Schedule(
                ...
                &DragonflyTopology::prediction_callback_for_MUSE,
                &dragonfly,
                0, // nodeRankID
                batch.batch,
    ...
    // 乱序安装节点
    std::vector<NS3NodeID> nodeIndex2rankID(serverCount);
    for (int node_index = 0; node_index < (int)serverCount; ++node_index)
    {
    ...
    for (int node_index = 0; node_index < (int)serverCount; ++node_index)
    {
        dragonfly.nodeIndex2rankID[node_index] = nodeIndex2rankID[node_index];
    }
    for (int node_index = 0; node_index < (int)serverCount; ++node_index)
    {
        dragonfly.rankID2nodeIndex[nodeIndex2rankID[node_index]] = node_index;
    }
    for (int rankID = 0; rankID < (int)dragonfly.serverCount; ++rankID)
    {
        int next_rankID = (rankID + 1) % dragonfly.serverCount;
        auto flow =
            make_pair(dragonfly.servers.Get(dragonfly.rankID2nodeIndex[rankID])->GetId(),
                      dragonfly.servers.Get(dragonfly.rankID2nodeIndex[next_rankID])->GetId());
        dragonfly.flows1.push_back(flow);
    }
    for (ns3::MPIRankIDType node_index = 0; node_index < serverCount; ++node_index)
    {
        auto node = dragonfly.servers.Get(node_index);
    ...
    // set serverID2Index
    for (auto i = 0; i < (int)serverCount; ++i)
    {
        dragonfly.serverID2Index[dragonfly.servers.Get(i)->GetId()] = i;
    }
