...
NS_LOG_COMPONENT_DEFINE ("FatTreeMoeMultiJobTest");
 ...
 * @brief 从全局事件列表提取特定 Rank 的任务流
 * Multi-Job Logic:
 * - Even Ranks run Job A (communicate only with Even Ranks)
 * - Odd Ranks run Job B (communicate only with Odd Ranks)
 */
std::vector<MoeTask> GetTasksForRank(MPIRankIDType rank, int num_nodes, int num_jobs) {
    std::vector<MoeTask> rank_tasks;
    
    // Safety check
    if (rank >= (MPIRankIDType)num_nodes) return rank_tasks;

    int my_job_id = rank % num_jobs;
    ...
    for (const auto& gev : g_scaled_events) {
        ...
        // 1. Fill Send Flows: My Rank -> Others (Filtered by Job ID)
        for (int dst = 0; dst < num_nodes; ++dst) {
            int dst_job_id = dst % num_jobs;
            // Isolation: Communicate only within same Job
            if (my_job_id == dst_job_id) {
        ...
        // 2. Fill Expected Recv: Others -> My Rank (Filtered by Job ID)
        for (int src = 0; src < num_nodes; ++src) {
             int src_job_id = src % num_jobs;
             if (my_job_id == src_job_id) {
...
{
    LogComponentEnable("FatTreeMoeMultiJobTest", LOG_LEVEL_INFO);
    ...
    std::string backoffStr = "1ms";      // Default Backoff
    int numJobs = 2;                     // Default: 2 Jobs
    ...
    cmd.AddValue("backoff", "Backoff time for fixed scheduler (e.g. 1ms, 100us)", backoffStr);
    cmd.AddValue("jobs", "Number of concurrent jobs (2 or 4)", numJobs);
    cmd.Parse(argc, argv);
    
    // Parse backoff time
    Time backoffTime = Time(backoffStr);
    double backoff = backoffTime.GetSeconds();

    NS_LOG_INFO("Configuring MoE Scheduling Test (" << num_nodes << " nodes)...");
    NS_LOG_INFO("Selected Scheduler: " << schedulerType);
    NS_LOG_INFO("Concurrent Jobs: " << numJobs);

    MoeJITApplicationHelper moeHelper;
    
    // (a) Load Scaled Trace
    moeHelper.SetTraceLoader([num_nodes, numJobs](MPIRankIDType rank) {
        return GetTasksForRank(rank, num_nodes, numJobs);
    });
    
    // (b) Select Scheduler
    if (schedulerType == "fixed") {
        NS_LOG_INFO("Using Fixed Probability Scheduler (P=" << prob << ", Backoff=" << backoff << "s)");
        auto fixedScheduler = std::make_shared<FixedProbScheduler>(prob, backoff);
