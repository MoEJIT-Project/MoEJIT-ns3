...
{
  public:
    void local_drain(bool value){ set_local_draining = value;} //zyy
    std::vector<std::vector<Ipv4Address>> packet_src_record; //zyy
    std::vector<std::vector<Ipv4Address>> packet_src_record_repetitive; //zyy
    uint32_t count_packet_src_record_repetitive = 0;
    void record_packet(const Ipv4Header& hdr); //zyy
    /**
  ...
  private:
    ...
    bool set_local_draining = false; //zyy
};
