...
TcpSocketBase::Send(Ptr<Packet> p, uint32_t flags)
{
    ...
    if (m_local_blocked)
    {
        if (UnAckDataCount() != 0)
        {
            // std::cout << "m_node" << GetNode()->GetId() << " unack: " << UnAckDataCount()
            //           << " prev_data: " << prev_data << " count: " << count << std::endl;
        }
        if (UnAckDataCount() == 0 || prev_data >= UnAckDataCount())
        {
            m_localDrainCallBack(GetNode()->GetId(), m_endPoint->GetPeerAddress());
        }
    }
