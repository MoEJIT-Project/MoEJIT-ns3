                        ...
                        NetDevice::PacketType packetType)
{
    ...
    if (ipv4Interface->IsUp() || set_local_draining)
    {
    ...
    {
        count_ ++;
        std :: cout << Simulator::Now() << " Dropping received packet -- interface is down " << count_ << std::endl;
                                   ...
                                   const Ipv4Header& header)
{
    ...
    for (auto mapIter = ttlMap.begin(); mapIter != ttlMap.end(); mapIter++)
    {
        ...
        if (ipHeader.GetTtl() == 0)
        {
            ...
            std::cout << "TTL exceeded.  Drop." << std::endl;
