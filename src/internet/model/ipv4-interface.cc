...
{
    NS_LOG_FUNCTION(this);
    // std::cout << "setdown (" << count_++ << "/all_links_should_be_down)" << std::endl;
    m_ifup = false;
}
...
void
Ipv4Interface::record_packet(const Ipv4Header& hdr) //zyy : only test once!!
{
    ...
    count_packet_src_record_repetitive += 1;
    if(packet_src_record_repetitive.size() < 1000){
        packet_src_record_repetitive.push_back({hdr.GetSource(), hdr.GetDestination()});
    }
    if(count_packet_src_record_repetitive >= 1000){
        count_packet_src_record_repetitive %= 1000;
        packet_src_record_repetitive[count_packet_src_record_repetitive] = {hdr.GetSource(), hdr.GetDestination()};
    }
    

    bool in = 0;
    for(auto pair : packet_src_record){
        if(pair[0] == hdr.GetSource() && pair[1] == hdr.GetDestination()){
            in = 1;
            break;
        }
    }
    if(in == 0)
        packet_src_record.push_back({hdr.GetSource(), hdr.GetDestination()});
}
